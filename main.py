...
def coletar_tudo(ano: int, mes: int) -> pd.DataFrame:
    ...
    # CEMADEN (manual — lê todos os arquivos .xls/.xlsx da pasta)
    ...
    try:
        ...
        if not df.empty:
            ...
            print(f"[AVISO] CEMADEN: nenhum arquivo .xls/.xlsx encontrado em {PASTA_CEMADEN_MANUAL}. "
...
def salvar_no_banco(df: pd.DataFrame):
    ...
    df.to_sql("dados_climaticos", conn, if_exists="append", index=False)
    conn.close()
    print(f"[OK] {len(df)} registros gravados em {CAMINHO_DB}")
